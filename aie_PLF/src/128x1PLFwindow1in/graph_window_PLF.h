...
public:
  ...
  WindowPLFGraph(unsigned int start_col, unsigned int start_row) {
    ...
    for (unsigned int i = 0; i < LANES_PER_GRAPH; i++) {
      ...
      adf::connect< adf::window<window_data_size+window_branch_size>   >(in_left_data[i],   k_mmul_left[i].in[0]);
      adf::connect< adf::window<window_data_size+window_branch_size>   >(in_right_data[i],   k_mmul_right[i].in[0]);
