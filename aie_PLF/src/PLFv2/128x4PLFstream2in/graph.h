...
public:
  ...
  PlioPLFGraph(){
    for(unsigned int i = 0; i < NUM_GRAPHS; i++) {
      ...
      adf::connect< adf::stream >(plio_in_EV[i].out[0], graphs[i].in_EV);
      ...
      for(unsigned int j = 0; j < LANES_PER_GRAPH; j++) {
        ...
        // branches
        plio_in_left_branch[idx]  = adf::input_plio::create(plio_name_branch("in_branch", i, 0, j), adf::plio_128_bits, data_name("stream/inputbranchleft", j));
        plio_in_right_branch[idx] = adf::input_plio::create(plio_name_branch("in_branch", i, 1, j), adf::plio_128_bits, data_name("stream/inputbranchright", j));
        adf::connect< adf::stream >(plio_in_left_branch[idx].out[0],  graphs[i].in_left_branch[j]);
        adf::connect< adf::stream >(plio_in_right_branch[idx].out[0], graphs[i].in_right_branch[j]);
