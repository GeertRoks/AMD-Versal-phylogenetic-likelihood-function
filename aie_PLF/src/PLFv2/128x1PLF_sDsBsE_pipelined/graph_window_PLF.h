...
public:
  ...
  WindowPLFGraph(unsigned int start_col, unsigned int start_row) {
    ...
    for (unsigned int i = 0; i < 4; i++) {
      ...
      adf::connect< adf::stream >(in_left_branch, k_mmul_left[i].in[1]);
      adf::connect< adf::stream >(in_right_branch, k_mmul_right[i].in[1]);
      ...
      adf::connect< adf::stream >(in_EV, k_mul_and_EV[i].in[2]);
