...
public:
  ...
  StreamPLFGraph(unsigned int start_col, unsigned int start_row) {
    ...
    for (unsigned int i = 0; i < LANES_PER_GRAPH; i++) {
      ...
      adf::connect< adf::stream >(in_left_branch[i], k_mmul_left[i].in[1]);
      adf::connect< adf::stream >(in_right_branch[i], k_mmul_right[i].in[1]);
      ...
      adf::connect< adf::stream >(in_EV, k_EV[i].in[1]);
